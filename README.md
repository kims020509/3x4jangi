...
2. 움직일 말의 영문과 함께 y좌표, x좌표 순서대로 입력한다. 
...
- input_command(board, catch) : 올바른 입력이면 입력을 바탕으로 old_xy, com_xy, type_, isMove를 반환한다. 아닐시 올바른 입력이 나올 때까지 반복한다.
...
플레이어 기준 (0 < x < 3 이 자신의 진영)으로 말 점수는 다음과 같다.
<center> 
...
Ai 기준 위치별 점수 : (100 + 2 + 2) - (100 + 2 + 1 + 3) = -2
...
이 값을 minimax 알고리즘의 key 값으로 설정하였다.
